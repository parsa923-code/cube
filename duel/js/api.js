(function () {
  ...
  function friendly(error) {
    ...
    console.error('[CubeDuel API] Supabase error:', error);
    if (error.code === '23505') return new Error('Conflict: already exists.');
    if (error.code === '23514' || (error.message && error.message.includes('players_player_number_check')))
      return new Error('Seat not allowed by database rules. Make sure the check constraint allows player_number 1–5.');
