(function () {
  ...
  async function refreshPlayers() {
    try {
      ...
      if (before === 2 && state.players.length === 1) toast('Opponent left.', 'info');
      if (before === 1 && state.players.length === 2) {
        toast('Opponent joined!', 'ok');
        const opp = state.players.find(p => p.id !== SB.uid());
        if (opp) Chat.setOpponentName(opp.name);
      }
      renderPlayers();
  ...
  function renderPresence(ps) {
    ...
    $('#oppStatus').textContent = on
      ...
      : (state.opp ? state.opp.name + ' · offline' : 'waiting…');
  ...
  function addTruncationNote() {
    ...
    tr.innerHTML = '<td colspan="3" class="trunc">… earlier rounds hidden</td>';
  ...
  function fillModal(row) {
    ...
    if (other && row.final_time != null && other.final_time != null) {
      const d = row.final_time - other.final_time;
      vs.textContent = (d < 0 ? 'You were faster by ' : 'Opponent faster by ') + (Math.abs(d) / 1000).toFixed(2) + 's';
