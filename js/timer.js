...
// Flow: hold SPACE (or touch) 0.5s -> READY -> release starts -> press stops.
// Optional WCA inspection: first tap starts inspection; >15s forces +2, >17s forces DNF.
window.CubeTimer = class CubeTimer {
    constructor(o) {
      this.digits = o.digits; this.deck = o.deck; this.pill = o.pill;
      this.onStart = o.onStart || (() => {});
      this.onFinish = o.onFinish || (() => {});
      this.isLocked = o.isLocked || (() => false);
      this.inspection = false;
      this.state = 'idle';
      this._t0 = 0; this._i0 = 0; this._insp = 0;
      this._holdT = null; this._raf = 0;
  
      document.addEventListener('keydown', e => this._keyDown(e));
      document.addEventListener('keyup', e => this._keyUp(e));
      this.deck.addEventListener('touchstart', e => {
        if (e.target.closest('button,.switch,input')) return;
        e.preventDefault(); this._press();
      }, { passive: false });
      ...
      this.deck.addEventListener('mousedown', e => {
        if (e.button === 0 && !e.target.closest('button,.switch,input')) this._press();
      });
      document.addEventListener('mouseup', () => this._release());
      if (o.btn) o.btn.addEventListener('click', e => { e.stopPropagation(); this._tap(); });
      if (o.resetBtn) o.resetBtn.addEventListener('click', e => { e.stopPropagation(); this.reset(); });
      this._render();
    }
    ...
    _press() {
      ...
      switch (this.state) {
        case 'running': {
          ...
          if (el < 120) return;            // anti-misclick guard
          this._finish(el);
          break;
        }
        case 'idle':
        case 'stopped':
          if (this.inspection) this._toInspection(); else this._toHold();
          break;
        case 'inspection': this._toHold(); break;
        default: break; // hold/ready: already pressing
      }
    }
    ...
    _tap() { // explicit button: quick start / stop (bypasses hold & inspection)
      if (this.isLocked()) return;
      if (this.state === 'running') {
        const el = performance.now() - this._t0;
        if (el >= 120) this._finish(el);
      } else {
        this._go();
      }
    }
    ...
    _toInspection() {
      ...
      this._i0 = performance.now(); this._insp = 0;
      this._loop();
    }
    ...
    _go() {
      ...
      this._insp = 0;
      if (forced === 'dnf') { this._finish(0, forced); return; } // >17s inspection
      this._setState('running');
      this._t0 = performance.now();
      this.onStart();
      this._loop();
      this._pendingForced = forced; // +2 carried from inspection
    }
    ...
    _setState(s) {
      this.state = s;
      this.deck.dataset.state = s;
      const label = { idle: 'IDLE', inspection: 'INSPECTION', hold: 'HOLD…',
        ready: 'READY', running: 'RUNNING', stopped: 'STOPPED' }[s] || s;
      this.pill.textContent = label;
    }
